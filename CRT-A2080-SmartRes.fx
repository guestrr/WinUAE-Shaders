...
#define h_sharp      2.50     // pixel sharpness (1.0 to 10.0)
...
{	
	...
	float3 color1 = (wl2*l2+wl1*l1+wr1*r1+wr2*r2)*wtt;
	
	float3 colmin = min(min(l2,l1),min(r1,r2));
	float3 colmax = max(max(l2,l1),max(r1,r2));
	
	if (cubic > 0.05) color1 = clamp(color1, colmin, colmax);
	
	l1*=l1; l1*=l1; r1*=r1; r1*=r1; l2*=l2; l2*=l2; r2*=r2; r2*=r2;
	float3 scolor1 = (sl2*l2+sl1*l1+sr1*r1+sr2*r2)*wts;
	...
	float3 color2 = (wl2*l2+wl1*l1+wr1*r1+wr2*r2)*wtt;
	...
	l1*=l1; l1*=l1; r1*=r1; r1*=r1; l2*=l2; l2*=l2; r2*=r2; r2*=r2;
	float3 scolor2 = (sl2*l2+sl1*l1+sr1*r1+sr2*r2)*wts;
