/*
   CRT - Arcade - Hires - SmartRes - Interlace shader
...
#define h_sharp      3.40     // pixel sharpness
...
{		
	...
	float wl4 = 3.5 + fp.x;  wl4*=0.5;  wl4*=wl4; wl4 = exp2(-h_sharp*wl4);
	float wl3 = 2.5 + fp.x;  wl3*=0.5;  wl3*=wl3; wl3 = exp2(-h_sharp*wl3);	
	float wl2 = 1.5 + fp.x;  wl2*=0.5;  wl2*=wl2; wl2 = exp2(-h_sharp*wl2);
	float wl1 = 0.5 + fp.x;  wl1*=0.5;  wl1*=wl1; wl1 = exp2(-h_sharp*wl1);
	float wct = 0.5 - fp.x;  wct*=0.5;  wct*=wct; wct = exp2(-h_sharp*wct);
	float wr1 = 1.5 - fp.x;  wr1*=0.5;  wr1*=wr1; wr1 = exp2(-h_sharp*wr1);
	float wr2 = 2.5 - fp.x;  wr2*=0.5;  wr2*=wr2; wr2 = exp2(-h_sharp*wr2);
	float wr3 = 3.5 - fp.x;  wr3*=0.5;  wr3*=wr3; wr3 = exp2(-h_sharp*wr3);
	float wr4 = 4.5 - fp.x;  wr4*=0.5;  wr4*=wr4; wr4 = exp2(-h_sharp*wr4);
	
	float wt = 1.0/(wl4+wl3+wl2+wl1+wct+wr1+wr2+wr3+wr4);

	float3 l4 = tex2D(decal, pC4 -x2-x2).xyz; l4*=l4;
	float3 l3 = tex2D(decal, pC4 -x2-dx).xyz; l3*=l3;	
	...
	float3 r3 = tex2D(decal, pC4 +x2+dx).xyz; r3*=r3;
	float3 r4 = tex2D(decal, pC4 +x2+x2).xyz; r4*=r4;
	
	float3 color1 = (l4*wl4 + l3*wl3 + l2*wl2 + l1*wl1 + ct*wct + r1*wr1 + r2*wr2 + r3*wr3 + r4*wr4)*wt;
	
	pC4+=dy;
	
	l4 = tex2D(decal, pC4 -x2-x2).xyz; l4*=l4;
	l3 = tex2D(decal, pC4 -x2-dx).xyz; l3*=l3;	
	l2 = tex2D(decal, pC4 -x2).xyz;    l2*=l2;
	l1 = tex2D(decal, pC4 -dx).xyz;    l1*=l1;
	ct = tex2D(decal, pC4    ).xyz;    ct*=ct;
	r1 = tex2D(decal, pC4 +dx).xyz;    r1*=r1;
	r2 = tex2D(decal, pC4 +x2).xyz;    r2*=r2;
	r3 = tex2D(decal, pC4 +x2+dx).xyz; r3*=r3;
	r4 = tex2D(decal, pC4 +x2+x2).xyz; r4*=r4;

	float3 color2 = (l4*wl4 + l3*wl3 + l2*wl2 + l1*wl1 + ct*wct + r1*wr1 + r2*wr2 + r3*wr3 + r4*wr4)*wt;	
