...
#define brightboost1 1.70     // adjust brightness - dark pixels (1.0 to 2.0)
...
#define scanline2    8.00     // scanline shape, edges (6.0 to 15.0)
#define beam_min     1.00     // dark area beam min - narrow (0.5 to 2.0)
#define beam_max     1.05     // bright area beam max - wide (0.5 to 2.0)
...
{	
	...
	wl4 = max(wl4 - zero, lerp(0.0,lerp(-0.10, -0.015, fp1),float(cubic > 0.05)));
	wl3 = max(wl3 - zero, lerp(0.0,lerp(-0.25, -0.100, fp1),float(cubic > 0.05)));
	wl2 = max(wl2 - zero, 0.0);
	wl1 = max(wl1 - zero, 0.0);
	wr1 = max(wr1 - zero, 0.0);	
	wr2 = max(wr2 - zero, 0.0);	
	wr3 = max(wr3 - zero, lerp(0.0,lerp(-0.25, -0.100, 1.-fp1),float(cubic > 0.05)));
	wr4 = max(wr4 - zero, lerp(0.0,lerp(-0.10, -0.015, 1.-fp1),float(cubic > 0.05)));
